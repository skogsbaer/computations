...
module Control.IncComps.CompEngine.SimpleStateIf (
  SimpleStateIf (..),
  SifState (..),
...
import Control.IncComps.CompEngine.SifCache (SifCache)
import qualified Control.IncComps.CompEngine.SifCache as SifCache
import Control.IncComps.CompEngine.Types
import Control.IncComps.CompEngine.Utils.DepMap
import qualified Control.IncComps.CompEngine.Utils.DepMap as DepMap
import qualified Control.IncComps.CompEngine.Utils.OutputsMap as OM
import qualified Control.IncComps.CompEngine.Utils.PriorityAgingQueue as Paq
...
trackOutputImpl
  ...
  -> AnyCompSinkOutsMap
  -> m ()
trackOutputImpl sif cap@CompAp{} outputs
                    ...
                    pure $
                      Just
                        ( "MULTIPLE_COMPAPS_ONE_OUTPUT: The output "
                            ...
                            ++ intercalate ", " (map show (SL.toList otherCapKeys))
                            ++ ". "
...
lookupCapResultImpl' cap@CompAp{} st =
...
capEvaluationStartedImpl :: SimpleStateIf m -> CompAp a -> m ()
...
dequeueGivenCapImpl :: SimpleStateIf m -> CompAp a -> m Bool
dequeueGivenCapImpl sif cap@CompAp{} =
