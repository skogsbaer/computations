...
module Control.IncComps.CompEngine.Tests.TestStateIf (
...
cCompDef =
  mkComp "c" inMemoryLHCaching $ \(_ :: Int) ->
    return ""
...
lCompDef
  ...
  -> CompDef Int r
lCompDef n cComp =
  mkComp n inMemoryLHCaching $ \(p :: Int) ->
    do
...
rCompDef cComp l1Comp l2Comp =
  mkComp "r" inMemoryLHCaching $ \(p :: Int) ->
    do
      Just x <- evalComp cComp p
...
fooCompDef =
  mkComp "foo" inMemoryLHCaching $ \(_ :: Int) ->
    return ""
...
barCompDef
  ...
  -> CompDef Int r
barCompDef foo =
  mkComp "bar" inMemoryLHCaching $ \(p :: Int) ->
    do
...
rootCompDef fooComp barComp =
  mkComp "root" inMemoryLHCaching $ \(p :: Int) ->
    do
