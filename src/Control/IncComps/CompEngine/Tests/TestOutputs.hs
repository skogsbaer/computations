...
module Control.IncComps.CompEngine.Tests.TestOutputs (htf_thisModulesTests) where
 ...
 where
  compDefs =
   ...
   where
    ...
    subCompDef =
      mkComp "sub" inMemoryShowCaching $ \b ->
        do
 ...
 where
  compDefs =
   ...
   where
    subCompDef :: CompDef () ()
    subCompDef =
      mkComp "sub" inMemoryShowCaching $ \() ->
        pureInfo "Generating dummy output" $
          put "output" "dummy"
 ...
 where
  compDefs =
   ...
   where
    subCompDef :: CompDef Bool ()
    subCompDef =
      mkComp "sub" inMemoryShowCaching $ \_ ->
 ...
 where
  compDefs =
   ...
   where
    ...
    commonCompDef =
      mkComp "common" inMemoryShowCaching $ \_ ->
        do
          flag <- liftM isYes (get "flag")
