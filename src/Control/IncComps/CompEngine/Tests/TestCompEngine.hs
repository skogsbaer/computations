...
module Control.IncComps.CompEngine.Tests.TestCompEngine (
  Id (..),
  NItem (..),
  CountRuns,
  SomeItem (..),
...
import Control.IncComps.CompEngine.Tests.TestDynamicChanges
...
wardGenDef =
  mkComp "wardGen" inMemoryShowCaching $ \wid ->
    do
...
allWardsCompDef wardGen =
  mkComp "allWards" inMemoryShowCaching $ \() ->
    do
...
test_changesWithinRun =
 ...
 where
  ...
  compCDef compA compB =
    mkComp "CompC" inMemoryLHCaching $ \_ ->
      do
  ...
  compBDef compA =
    mkComp "CompB" inMemoryLHCaching $ \_ ->
      do
        Just msg <- evalComp compA ()
        return (take 5 msg)
  compADef :: CompDef () String
  compADef =
    mkComp "CompA" inMemoryLHCaching $ \_ ->
      do
        msg <- requestExt
...
test_revive =
 ...
 where
  compCDef
    ...
    -> Comp String String
    -> CompDef () ()
  compCDef compA compB = mkComp "CompC" inMemoryLHCaching $ \_ -> do
  ...
  compBDef = mkComp "CompB" inMemoryLHCaching $ \_ -> requestExt
  compADef :: CompDef () String
  compADef = mkComp "CompA" inMemoryLHCaching $ \_ -> requestExt
  getComps =
    do
      compA <- defineComp (compADef)
